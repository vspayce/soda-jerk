...
walk sheet in both colours, and the two sprays) and prints the stride to
...
from rig import build, load, stride

names = sys.argv[1:] or list(CHARACTERS)
for name in names:
    cfg = CHARACTERS[name]
    w, h = build(cfg)
    print(f"{name:9s} {cfg['names']['portrait']:8s} canvas {w}x{h}  stride {stride(cfg, load(cfg)):.3f}")
if not sys.argv[1:]:
